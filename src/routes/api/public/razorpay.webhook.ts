        ...
        if (event === "payment.captured" || event === "order.paid") {
          ...
          try {
            ...
            const data = {
              ...
              requests: `${booking.special_requests ?? ""}${booking.special_requests ? " · " : ""}Rooms: ${roomsBreakdown} · Subtotal ₹${subtotal.toLocaleString("en-IN")} + GST ₹${taxes.toLocaleString("en-IN")} = ₹${grandTotal.toLocaleString("en-IN")} (${booking.nights || 1} night${(booking.nights || 1) > 1 ? "s" : ""}) · Payment ID ${paymentId}`,
            };
            await sendEmails([
              {
